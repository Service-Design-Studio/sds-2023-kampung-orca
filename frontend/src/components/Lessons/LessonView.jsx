...
function SlideEx() {
  //This is the chat function button. Ideally, i'll move it to a different document later
...
const Lesson = () => {
  ...
  useEffect(() => {
    const url = `${process.env.REACT_APP_GATEWAY_URL}/api/v1/page/show/${params.lesson_id}`;
    fetch(url)
      .then((res) => {
        if (res.ok) {
          return res.json();
          ...
          >
            <Stack align="flex-end">
              <Link to={`/lesson-complete/${pages.page_id}`}>
                <Button size="lg" colorScheme="green" height="48px" shadow="md">
                  Complete Lesson
                </Button>
              </Link>
            </Stack>
