...
class ApplicationController < ActionController::API
  ...
  def current_user
    if ENV["ADMIN"] == @token && Rails.env.development?
      ...
      @current_user = {token: "admin", user_id: "admin"} # modify this line to change the user account for Mohammad
    elsif ENV["NORMAL_USER2"] == @token && Rails.env.development?
      @current_user = {token: "admin", user_id: "admin"} # modify this line to change the user account for ALoysius
