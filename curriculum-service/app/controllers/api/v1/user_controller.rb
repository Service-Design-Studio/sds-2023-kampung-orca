class Api::V1::UserController < ApplicationController
  ...
  def create
    user = User.create!(user_id: params[:user_id], lessons_access: [], exercises_access:[])
    if user
      head 202
    else
  ...
  def open_next_lesson
    ...
    next_exercise = Exercise.find_by(params[next_lesson.lesson_id]) if next_lesson
    unless user.exercises_access.include?(next_exercise.exercise_id)
      user.exercises_access << next_exercise.exercise_id if next_exercise
    end
